...
/**
 * A 2-dimensional Manhattan Distance heuristic implementation, for tile-based
 * maps.
 * 
 ...
 */
public abstract class Manhattan2D {

    /**
     * A 2-dimensional Manhattan Distance heuristic implementation for
    ...
    public static class Double implements Heuristic<Point2D.Double> {
        ...
        public float getHeuristicCost(final Map<Point2D.Double> map, final MovingEntity entity, final Point2D.Double source, final Point2D.Double target) {
            return (float) (Math.abs(source.x - target.x) + Math.abs(source.y - target.y));
        }
    }
}